...
export default {
  ...
  data() {
    return {
      ...
      search: "",
      dataTitle: "Daya Listrik",
    };
  },

  //hook "asyncData"
  async asyncData({ store }) {
    await store.dispatch("operator/dayaListrik/getAllDataState");
  },
  ...
  watch: {
    search(newSearchValue) {
      ...
      this.$store.dispatch(
        "operator/dayaListrik/getAllDataState",
        newSearchValue
      );
    },
  },
      ...
      if (confirmationResult.isConfirmed) {
        try {
          const response = await this.$axios.delete(
            `/api/v1/daya-listrik/${item.id}/hapus`
          );
