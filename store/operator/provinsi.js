...
export const actions = {
  ...
  getAllDataState({ commit, state }, payload) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.$axios.get(`/api/v1/provinsi?name=${search}&page=${state.page}`)
        //success
        .then((response) => {
  ...
  getDetailIdstate({ commit }, payload) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.$axios.get(`/api/v1/provinsi/${payload}`)
  ...
  updateDataState({ dispatch, commit }, { provinsiId, payload }) {

    //set promise
    return new Promise((resolve, reject) => {

      //store to Rest API "/api/admin/categories/:id" with method "POST"
      this.$axios.post(`/api/v1/provinsi/${provinsiId}/ubah`, payload)
